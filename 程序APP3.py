            ...
            try:
                ...
                try:
                    ...
                    if isinstance(shap_values, list):
                        ...
                        plt.figure(figsize=(10, 3))
                        shap.force_plot(
                            explainer.expected_value[class_index] if isinstance(explainer.expected_value, list) else explainer.expected_value,
                            shap_values[:, :, class_index],
                            features_df,
                            matplotlib=True,
                            show=False
                    ...
                    <div style="background-color: #f8f9fa; padding: 10px; border-radius: 5px; margin-top: 10px; font-size: 0.9rem;">
                      <p><strong>图表解释:</strong> 上图显示了各个特征如何影响预测结果。红色表示增加死亡风险的特征，蓝色表示降低死亡风险的特征。特征值的宽度表示其影响程度。</p>
                    </div>
                    """, unsafe_allow_html=True)
                    
                except Exception as shap_error:
                    st.error(f"生成SHAP图时出错: {str(shap_error)}")
                    st.info("无法生成SHAP解释图，请联系技术支持。")
